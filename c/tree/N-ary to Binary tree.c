...
void AfficherBinaireU(struct Noeud *racine, int espace)
{
    ...
    print2DU(racine->fils[1], espace); 		// Process fils[1] premier fils
    ...
    print2DU(racine->fils[0], espace);   	// Process fils[0]
}
...
{	
	...
	while(i < nbFils)
		{
			struct Noeud *filsCurr = encoder(racine->fils[i]);		//pointer le noeud "racine->fils[i]" de l'arbre n-air
			if(i == 0){
