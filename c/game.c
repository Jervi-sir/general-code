...
long long int argtoll(const char *str, const char **end, stoll_t stoll)
{
...
static inline double ns_to_us(double val)
{
...
static inline double ns_to_ms(double val)
{
...
static inline double delta_ns(struct timespec *a, struct timespec *b)
{
...
static inline double delta_us(struct timespec *a, struct timespec *b)
{
...
static inline double delta_ms(struct timespec *a, struct timespec *b)
{
...
static inline float check_hit(float a, float b, float d)
{
...
static inline bool check_in(float v, float a, float b)
{
...
uint32_t color_mix(uint32_t c1, uint32_t c2, float m)
{
	...
	return ((r & 0xFF) << 24) | ((g1 & 0xFF) << 16) | ((g2 & 0xFF) << 8) | ((b & 0xFF) << 0);
...
void animate(state_t * state, unsigned steps)
{
	...
	while (steps-- > 0)
	{
		...
		case S_INITIAL:
			...
			if ((state->ls.score == 99) || (state->rs.score == 99))
		...
		case S_GAME:
			...
			if ((state->b.hpos_f - state->b.hsize) <= (state->l.hpos + state->l.hsize))
			{

				float hit = check_hit(state->b.vpos_f,
			...
			if ((state->b.hpos_f + state->b.hsize) >= (state->r.hpos - state->r.hsize))
			{
		...
		if ((state->b.vpos_f >=
			...
			(state->b.vpos_f <= (state->f.vmin + fabs(state->b.vvec_f))))
		{
...
void display(state_t * state)
{
	...
	if (check_in(state->l.hpos, state->f.hmin, state->f.hmax))
		...
		set_pon_reg(PON_REG_LHPOS,
					((state->l.hpos + state->l.hsize) << 16) | (state->l.hpos - state->l.hsize));
	...
	if (check_in(state->l.vpos, state->f.vmin, state->f.vmax))
		...
		set_pon_reg(PON_REG_LVPOS,
					((state->l.vpos + state->l.vsize) << 16) | (state->l.vpos - state->l.vsize));
	...
	if (check_in(state->r.hpos, state->f.hmin, state->f.hmax))
		...
		set_pon_reg(PON_REG_RHPOS,
					((state->r.hpos + state->r.hsize) << 16) | (state->r.hpos - state->r.hsize));
	...
	if (check_in(state->r.vpos, state->f.vmin, state->f.vmax))
		...
		set_pon_reg(PON_REG_RVPOS,
					((state->r.vpos + state->r.vsize) << 16) | (state->r.vpos - state->r.vsize));
	...
	if (check_in(state->b.hpos, state->f.hmin, state->f.hmax))
		...
		set_pon_reg(PON_REG_BHPOS,
					((state->b.hpos + state->b.hsize) << 16) | (state->b.hpos - state->b.hsize));
	...
	if (check_in(state->b.vpos, state->f.vmin, state->f.vmax))
		...
		set_pon_reg(PON_REG_BVPOS,
					((state->b.vpos + state->b.vsize) << 16) | (state->b.vpos - state->b.vsize));
	...
	if (state->a.cnt_bounce)
		...
		bcolor = color_mix(bcolor, state->a.col_bounce, state->a.cnt_bounce * 1.0 / CNT_BOUNCE);

	if (state->a.cnt_fade)

		bcolor = color_mix(state->a.col_fade, bcolor, state->a.cnt_fade * 1.0 / CNT_FADE);
	...
	set_pon_reg(PON_REG_LSCOLOR, color_mix(0xFFFFFFFF, state->a.col_lflash,
										   state->a.cnt_lflash * 1.0 / CNT_FLASH));


	set_pon_reg(PON_REG_RSCOLOR, color_mix(0xFFFFFFFF, state->a.col_rflash,
...
int main(int argc, char *argv[])
{
	...
	if (base == (void *)-1)
	{
		...
		fprintf(stderr,
				...
				(long)cmv_base, (long)cmv_size, (long)cmv_addr, strerror(errno));
	...
	fprintf(stderr,
			...
			(long unsigned)cmv_base, (long unsigned)cmv_size, (long unsigned)cmv_addr);
	...
	if (scnb == (void *)-1)
	{
		...
		fprintf(stderr,
				"error mapping 0x%08lX+0x%08lX @0x%08lX.\n%s\n",
				(long)scn_base, (long)scn_size, (long)scn_addr, strerror(errno));
	...
	fprintf(stderr,
			"mapped 0x%08lX+0x%08lX to 0x%08lX.\n",
			(long unsigned)scn_base, (long unsigned)scn_size, (long unsigned)scn_addr);
	...
	fprintf(stderr,
			"mapped 0x%08lX+0x%08lX to 0x%08lX.\n",
			(long unsigned)map_base, (long unsigned)map_size, (long unsigned)map_addr);
	...
	if (ioctl(i2c, I2C_SLAVE, i2c_addr) < 0)
	{
		...
		fprintf(stderr, "error setting slave address 0x%02x.\n%s\n", i2c_addr, strerror(errno));
	...
	it_spec.it_interval = (struct timespec)
	{
	0, 1000000000L / num_fps};

	it_spec.it_value = (struct timespec)
	{
	0, 1};
	...
	while (1)
	{
		...
		int num = select(it + 1, &read_set, NULL, NULL, NULL);
		...
		if (FD_ISSET(it, &read_set))
		{
			...
			int cnt = read(it, &expired, sizeof(expired));
